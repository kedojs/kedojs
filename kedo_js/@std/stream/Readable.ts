...
function extractHighWaterMarkFromQueuingStrategyInit(
  ...
  if (highWaterMark === undefined) {
    throw new TypeError("Queuing strategy must have a highWaterMark property");
  }
...
class ByteLengthQueuingStrategy implements QueuingStrategy {
  ...
  constructor(init: QueueingStrategyInit) {
    this[_highWaterMark] = extractHighWaterMarkFromQueuingStrategyInit(init);
...
function readableStreamResource(
  ...
  // asyncOp(op_wait_close_readable_stream, resource, false).then(
  //   () => reader.cancel("Resource stream closed"),
  //   () => {},
  // );
...
const setUpReadableByteStreamController = (
  ...
  if (autoAllocateChunkSize !== undefined) {
    ...
    if (autoAllocateChunkSize <= 0)
      throw new RangeError("Auto allocate chunk size must be greater than 0");
  }
  ...
  startPromise.then(
    () => {
      ...
      assert(controller[_pullAgain] === false, "Controller is pulling again");
...
const RedableStreamAsyncIteratorPrototype = Object.setPrototypeOf(
  {
    ...
    return() {
      ...
      assert(reader[_readRequests].length === 0, "Read requests must be empty");
      // 4. If iterator’s prevent cancel is false:
      if (this[_preventCancel] === false) {
        // 4.1. Let result be ! ReadableStreamReaderGenericCancel(reader, arg).
        const result = readableStreamReaderGenericCancel(reader, undefined);
...
class ReadableStream {
  ...
  [_detached]: boolean;
  [_disturbed]: boolean;
  [_reader]: ReadableStreamDefaultReader | ReadableStreamBYOBReader | undefined;
  ...
  constructor(
    ...
    const underlyingSourceDict = (underlyingSource as UnderlyingSource) || {};
  ...
  getReader<T = ReadableStreamDefaultReader | ReadableStreamBYOBReader>(options?: {
    mode: "byob";
  }): T {
    //x
    assert(
...
function readableStreamBYOBReaderErrorReadIntoRequests(
  ...
  readIntoRequests.forEach((readIntoRequest) => readIntoRequest.errorSteps(e));
}
...
const readableByteStreamControllerHandleQueueDrain = (
  ...
  assert(controller[_stream][_state] === "readable", "Stream is not readable");
...
const setUpReadableStreamBYOBReader = (
  ...
  if (
    !isPrototypeOf(stream[_controller], ReadableByteStreamController.prototype)
  )
    throw new TypeError("Stream controller is not a byte stream controller");
...
const readableByteStreamControllerCommitPullIntoDescriptor = (
  ...
  if (pullIntoDescriptor.readerType === "default") {
    ...
    assert(pullIntoDescriptor.readerType === "byob", "Reader type is not byob");
...
const readableByteStreamControllerRespondWithNewView = (
  ...
  if (state === "closed") {
    ...
    if (view.byteLength !== 0) throw new TypeError("View byteLength is not 0");
  } else {
    // 6.1 Assert: state is "readable".
    assert(state === "readable", "Stream is not readable");
...
class ReadableStreamBYOBRequest {
  ...
  respond(bytesWritten: number) {
    ...
    assert(this[_view]!.buffer.byteLength > 0, "View buffer byteLength is 0");
...
const readableByteStreamControllerClose = (
  ...
  if (controller[_pendingPullIntos].length > 0) {
    ...
    if (
      ...
      const e = new TypeError("Bytes filled is not divisible by element size");
...
class ReadableByteStreamController {
  ...
  enqueue(chunk: ArrayBufferView): void {
    ...
    if (chunk.byteLength === 0) throw new TypeError("Chunk byteLength is 0");
    ...
    if (this[_closeRequested]) throw new TypeError("Close requested is true");
...
export {
  ...
  CountQueuingStrategy, createReadableByteStream,
  createReadableStream,
  isDisturbed,
  isErrored,
  isInReadableState, ReadableByteStreamController,
  ReadableStream,
  ReadableStreamBYOBReader,
  ReadableStreamBYOBRequest, readableStreamClose,
  readableStreamCloseByteController, ReadableStreamDefaultController,
  ReadableStreamDefaultReader, readableStreamEnqueue, ReadableStreamReaderMode, readableStreamResource, ReadableStreamType
};
