...
use crate::{
    http::{fetch::lib::FetchModule, server::lib::server_exports},
    signals::signal_exports,
    streams::streams::StreamResourceModule,
};

use super::{text_encoding::TextEncodingModule, url_utils::UrlUtilModule};
...
pub fn util_exports(ctx: &JSContext) -> JSObject {
    ...
    UrlUtilModule::export(ctx, &exports).expect("Failed to export UrlUtilModule");
    TextEncodingModule::export(ctx, &exports)
        .expect("Failed to export TextEncodingModule");
    FileSystemModule::export(ctx, &exports).expect("Failed to export FileSystemModule");
