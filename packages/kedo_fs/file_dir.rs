...
impl FsDirEntry {
    pub fn as_object(&self, ctx: &JSContext) -> JSResult<JSObject> {
        ...
        object.set_property("parentPath", &parent_path, Default::default())?;

        let name = JSValue::string(ctx, self.name.clone());
        object.set_property("name", &name, Default::default())?;

        let is_dir = JSValue::boolean(ctx, self.is_dir);
        object.set_property("isDir", &is_dir, Default::default())?;

        let is_file = JSValue::boolean(ctx, self.is_file);
        object.set_property("isFile", &is_file, Default::default())?;

        let is_symlink = JSValue::boolean(ctx, self.is_symlink);
        object.set_property("isSymlink", &is_symlink, Default::default())?;
