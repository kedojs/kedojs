...
async function testReadableStreamResource() {
  ...
  let result = await asyncOp(op_read_readable_stream, reader);
  assert.ok(
    result[0] === 6 && result[1] === 5 && result[2] === 6,
    `Expected [1, 2, 3], got ${result}`,
  );

  result = await asyncOp(op_read_readable_stream, reader);
  assert.ok(
...
async function testReadableStreamResourceBenchmark() {
  ...
  while (result = await asyncOp(op_read_readable_stream, reader)) {
    // Do nothing
  }
  let end = Date.now();
