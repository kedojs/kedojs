...
async function testFetchBasicGetRequest() {
    ...
    const response = await fetch(`${BASE_URL}/get`, { headers: { "User-Agent": "curl/8.7.1" } });
    strictEqual(response.status, 200, "Status should be 200 for GET request");
    ok(response.headers instanceof Headers, "Headers should be an instance of Headers");

    const data = await response.json();
    strictEqual(data.url, `${BASE_URL}/get`, "URL in response should match the request URL");
}

async function testHpttRequest() {
    // Test 1: Basic GET request
    const response = await fetch(`${BASE_HTTP_URL}/get`, { headers: { "User-Agent": "curl/8.7.1" } });
    strictEqual(response.status, 200, "Status should be 200 for GET request");
    const data = await response.json();
    strictEqual(data.url, `${BASE_HTTP_URL}/get`, "URL in response should match the request URL");
}
...
async function testFetchPostRequestWithBody() {
    ...
    const data = await response.json();
    deepStrictEqual(data.json, payload, "Server should receive the correct JSON body");
}
...
async function testFetchRequestHeaders() {
    ...
    strictEqual(response.status, 200, "Status should be 200 when sending headers");
    const data = await response.json();
    strictEqual(
...
async function testFetchResponseHeaders() {
    ...
    const response = await fetch(`${BASE_URL}/response-headers?X-Test-Header=TestValue`);
    strictEqual(response.status, 200, "Status should be 200 when fetching response headers");
    strictEqual(
...
async function testFetchRedirectHandling() {
    ...
    const data = await response.json();
    strictEqual(data.url, `${BASE_URL}/get`, "URL should be updated after redirect");
}

async function testFetchNoRedirect() {
    // Test 8: Handling manual redirect mode
    const response = await fetch(`${BASE_URL}/redirect/1`, { redirect: "manual" });
    strictEqual(response.status, 302, "Status should be 302 when redirects are manual");
    strictEqual(response.headers.get("Location"), `/get`, "Location header should be set");
}
...
async function testFetchRequestMethods() {
    ...
    for (const method of methods) {
        ...
        strictEqual(response.status, 200, `Status should be 200 for ${method} request`);
        const data = await response.json();
...
async function testFetchAbortController() {
    ...
    setTimeout(() => { controller.abort() }, 100); // Abort after 10ms
    await rejects(
...
async function testFetchResponseStreamJson() {
    ...
    strictEqual(listItems.length, 10, "Should have received 10 bytes from stream");
}

async function testFetchCredentials() {
    // Test 16: Fetch with credentials
    const response = await fetch(`${BASE_URL}/cookies`, { credentials: "include" });
    strictEqual(response.status, 200, "Status should be 200 when sending credentials");
}

async function testFetchCacheMode() {
    // Test 17: Fetch with cache mode
    const response = await fetch(`${BASE_URL}/cache`, { cache: "no-store" });
    strictEqual(response.status, 200, "Status should be 200 when using cache mode");
}
...
async function testFetchCustomRequest() {
    ...
    strictEqual(response.status, 200, "Status should be 200 when using custom Request");
    const data = await response.json();
    strictEqual(
        data.headers["X-Custom-Header"],
        "TestValue",
...
async function testFetchStreamResponse() {
    ...
    strictEqual(response.status, 200, "Status should be 200 for streamed response");
...
async function testFetchMedia() {
    ...
    ok(array instanceof ArrayBuffer, "Response should be parsed as an ArrayBuffer");
}
...
async function testFetchWithCredentialsOmit() {
    ...
    strictEqual(response.status, 200, "Status should be 200 when credentials are omitted");
}
...
async function testFetchJSONResponse() {
    ...
    strictEqual(response.headers.get("Content-Type"), "application/json", "Content-Type should be JSON");
}
