use super::fetch_decoder::Decoder;
use super::fetch_decoder::ResponseStream;
use super::fetch_errors::FetchError;
use super::util::{basic_auth, extract_authority, remove_credentials};
use bytes::Bytes;
...
use super::request::{FetchRequest, RequestBody, RequestRedirect};
use super::response::FetchResponse;
use super::HeadersMap;
use crate::streams::streams::InternalStreamResourceReader;

type FetchBodyStream =
    StreamBody<MapOk<InternalStreamResourceReader<Vec<u8>>, fn(Vec<u8>) -> Frame<Bytes>>>;
type FetchBody = Either<FetchBodyStream, BoxBody<bytes::Bytes, Infallible>>;

fn into_bytes_frame(chunk: Vec<u8>) -> Frame<Bytes> {
...
impl TryFrom<&mut FetchRequest> for Request<FetchBody> {
    ...
    fn try_from(fetch_request: &mut FetchRequest) -> Result<Self, Self::Error> {
        ...
        // Set headers
        for (name, value) in &fetch_request.headers.inner {
            ...
            RequestBody::Stream(stream) => match stream.take() {
                Some(stream) => {
                    let body = StreamBody::new(
                        stream.map_ok(into_bytes_frame as fn(Vec<u8>) -> Frame<Bytes>),
                    );
                    Either::Left(body)
                }
...
impl Future for PendingRequest {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
        ...
        let body_stream = ResponseStream::new(response.into_body());
        let decoder_body = Decoder::detect(body_stream, &headers);
        let fetch_response = FetchResponse {
            ...
            headers,
            aborted: false,
            body: Some(Box::new(decoder_body)),
        };
...
mod tests {
    use super::*;
    use crate::http::test_utils::test_utils::start_test_server;
    use crate::{
