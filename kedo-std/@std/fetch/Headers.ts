...
type HeadersGuard = 'none' | 'immutable' | 'request' | 'request-no-cors' | 'response';

const isHTTPWhitespace = (characterCode: number) => {
    // Checks for HTTP tab (0x09), space (0x20), LF (0x0A), and CR (0x0D)
    return characterCode === 0x09 || characterCode === 0x20 || characterCode === 0x0A || characterCode === 0x0D;
}
    ...
    if (Array.isArray(headersInit)) {
        for (let i = 0; i < headersInit.length; i++) {
            if (headersInit[i].length !== 2) {
                throw new TypeError('Header sub-sequence must contain exactly two items');
            }
...
const fillHeadersMapFrom = (headers: HeadersInit, headersMap: Headers, headersGuard: HeadersGuard = 'none') => {
    fillHeadersMap(headers, headersMap);
    headersMap[_headersGuard] = headersGuard;
}

const appendSetCookie = (value: string, setCookieValues: string[], headersGuard: HeadersGuard) => {
...
const appendToHeaderMap = (headersMap: Headers, name: string, value: string) => {
    const normalizedValue = value.trim();
    if (name === 'set-cookie')
        return appendSetCookie(normalizedValue, headersMap[_setCookieValues], headersMap[_headersGuard]);
    ...
    if (!isValidHTTPToken(name))
        throw new TypeError("Header name is invalid");

    if (headersMap[_headersGuard] === 'immutable')
        throw new TypeError("Headers object is immutable");


    Array.prototype.push.call(headersMap[_headerList], [lowerCase(name), normalizedValue]);
}
...
class Headers {
    ...
    has(name: string) {
        ...
        if (name === 'set-cookie')
            return this[_setCookieValues].length > 0;
    ...
    set(name: string, value: string) {
        ...
        if (name === 'set-cookie') {
            this[_setCookieValues] = [];
            return appendSetCookie(value, this[_setCookieValues], this[_headersGuard]);
        }
    ...
    forEach(callback: ForEachCallback, thisArg?: any) {
        for (let i = 0; i < this[_headerList].length; i++) {
            callback.call(thisArg, this[_headerList][i][1], this[_headerList][i][0], this);
        }

        if (this[_setCookieValues].length > 0) {
            callback.call(thisArg, this[_setCookieValues].join(', '), 'set-cookie', this);
        }
    }
...
export { Headers, fillHeadersMapFrom };
